...
descripcion_button.addEventListener("click", () => {
  ...
  window1.style.display = "none";// para ocultar la primera ventana se utlizo el atributo none
  window2.style.display = "block";//para poder ver  la segunda ventana se utlizo el a tributo block
})
...
function acapTextOffset() {
  let offset = document.getElementById("interval").value; // NUMEROS DE CUANTO  VA A CORRER
  let string = document.getElementById("c1").value; // MENSAJE DEL USUARIO
  const decodeString = cipher.decode(offset, string); //LLAMADO CIPHER DECODE
